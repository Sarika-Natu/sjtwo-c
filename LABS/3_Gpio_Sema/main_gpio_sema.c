...
static void vInit(void *task_parameter);
...
static void vControlLED_Flashy(void *task_parameter) {
    while (true) {
        ...
        if (xSemaphoreTake(flashy_easter_egg, 1000)) {
            ...
            for (u_int8_t index = 1; index < 4; index++) {
                vCtrlLed(port_leds[index].port, port_leds[index].pin, true);
                vTaskDelay(FLASHY_DELAY);
                vCtrlLed(port_leds[index].port, port_leds[index].pin, false);
                vTaskDelay(FLASHY_DELAY);
...
int main(void) {
    ...
    xTaskCreate(vInit, "vInit", configMINIMAL_STACK_SIZE, NULL, PRIORITY_LOW, NULL);
    xTaskCreate(vBlinkLED, "vBlinkLED", configMINIMAL_STACK_SIZE, (void *)&port1_led8, PRIORITY_LOW, NULL);
    xTaskCreate(vControlLED, "vControlLED", configMINIMAL_STACK_SIZE, (void *)&port1_led8, PRIORITY_HIGH, NULL);
    xTaskCreate(vReadSwitch, "vReadSwitch", configMINIMAL_STACK_SIZE, (void *)&port_switches, PRIORITY_LOW, NULL);
    xTaskCreate(vControlLED_Flashy, "vControlLED_Flashy", configMINIMAL_STACK_SIZE, (void *)&flashy_led, PRIORITY_LOW,
                NULL);
    xTaskCreate(vReadSwitch_Flashy, "vReadSwitch_Flashy", configMINIMAL_STACK_SIZE, (void *)&port_switches, PRIORITY_LOW,
                NULL);
